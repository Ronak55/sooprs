...
const ClientProfile = ({navigation, route}: {navigation: any; route: any;}) => {

  // const {img, name, role, rating} = route.params;

  const tabs = ['Company Info', 'Projects', 'Reviews'];
  const [isClient, setisClient] = useState(true);

  const profileInfo = [
    {
      img: Images.maleImageOne,
      name: "Leonardo",
      role: "UI/UX Designer",
      rating: "4.2",
      heading : "Bio", 
  ...
  return (
    <View style={styles.container}>
      ...
      <ProfileTabs tabs={tabs} isClient={isClient} services={[]} skills={[]}/>
      </ScrollView>
    </View>
  );
};
