import {
  ...
  const AllProjects = ({navigation}: {navigation: any}) => {
    const [projectDetails, setProjectDetails] = useState([]); // Store project data
      ...
      try {
        ...
        if (responseData.status === 200) {
          if (responseData.msg.length === 0) {
            ...
            // Append new data to existing project details
    ...
    const loadMoreProjects = () => {
      const newOffset = offset + 1; // Increment offset
      setOffset(newOffset);
    ...
    const renderFooter = () => {
      if (isLoading) {
        return <ActivityIndicator size="large" color={Colors.primary} style={{marginVertical: hp(2)}} />;
      }
  
      if (hasMore) {
        return (
          <TouchableOpacity style={styles.showMoreButton} onPress={loadMoreProjects}>
